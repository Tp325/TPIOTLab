...
float Sensor::readBat() {
  return readVotage(BatSense3V3) * 1.27 - 0.2;
}
...
float Sensor ::getOxygen() {
  for (int i = 0; i < 5; i++) {
    O2Raw[i] = (getVoltage(sentToSensor("DO_2_1")) * O2Slope + O2Intercept) < 0.1 ? 0 : (getVoltage(sentToSensor("DO_2_1")) * O2Slope + O2Intercept);
  }
