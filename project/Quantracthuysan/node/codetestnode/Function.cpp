...
void TransmitToSink::receive() {
  deserializeJson(Stored, Serial.readString());
...
void TransmitToSink::send(Sensor sensor) {
  ...
  if (this->id == this->calledId) {
    ...
    dataSendToSink["Data"] = (sensor.readAnalogFromSensor()*5/1023)*16.8;
  }
...
void TransmitToSink::processDataToSink(Sensor sensor) {
  ...
  Serial.println((sensor.readAnalogFromSensor()*5/1023)*16.8);
